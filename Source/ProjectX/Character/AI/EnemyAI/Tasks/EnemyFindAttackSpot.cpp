...
UEnemyFindAttackSpot::UEnemyFindAttackSpot()
{
	static ConstructorHelpers::FObjectFinder<UEnvQuery> EQS_Query(TEXT("EnvQuery'/Game/Game/Characters/AI/AI_Enemy/EnvQueries/EQ_EnemyGetAvailableHideSpots'"));
...
EBTNodeResult::Type UEnemyFindAttackSpot::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	QueryRequest.Execute(EEnvQueryRunMode::AllMatching, this, &UEnemyFindAttackSpot::QueryFinished);
...
void UEnemyFindAttackSpot::QueryFinished(TSharedPtr<FEnvQueryResult> Result)
{
	UE_LOG(LogTemp, Warning, TEXT("ENEMY C++ EQS Completed"));

	TArray<AActor*> AllDetectedActors;
	Result->GetAllAsActors(AllDetectedActors);

	UE_LOG(LogTemp, Warning, TEXT("ENEMY EQS %i"), AllDetectedActors.Num());
	...
	if (AI)
	{
		if (AllDetectedActors.Num())
		{
			...
			for (int32 i = 0; i < AllDetectedActors.Num(); ++i)
			{
				...
				DrawDebugSphere(GetWorld(), AllDetectedActors[i]->GetActorLocation(), 100.f, 8, FColor::Green, false, 1.0f);
